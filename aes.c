...
// Simple S-Box (not secure, just for structure)
...
void encrypt_block(const uint8_t* input, uint8_t* output, const uint8_t* key) {
    ...
    for (int round = 0; round < 10; round++) {
        ...
        // MixColumns (omitted for brevity, just XORs)
        for (int i = 0; i < 16; i++) {
            state[i] ^= key[(round * 16 + i) % 16]; // Simple key schedule
        }
    }
...
int main() {
    ...
    encrypt_block(input, output, key);

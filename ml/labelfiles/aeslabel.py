...
# ============================================================
# AES KEY SIZE DETECTION FROM FILENAME
# ============================================================
...
def extract_features(func):
    ...
    f["rsa_bigint_detected"]= bool(cs.get("rsa_bigint_detected"))  # kept for consistency
...
def process():
    ...
    for jf in files:
        ...
        for func in data.get("functions",[]):
            ...
            # ---------- 1. FUNCTION NAME BASED AES DETECTION ----------
            is_aes_func = any(key in lname for key in AES_FUNCS)

            # ---------- 2. KEY SIZE FROM FILENAME ----------
            if is_aes_func and keysize is not None:
                label = keysize          # AES-128 / AES-192 / AES-256
            else:
                label = "Non-Crypto"
    ...
    print("[+] Generated AES dataset:", OUTPUT_JSON, OUTPUT_CSV)
